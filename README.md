...
我們用政府公開資料分析各縣市高齡人口（需求）和長照資源（供給）是否平衡，
...
隨機森林表現最好。特徵重要度由高到低是：身障老人占比（0.30）、老年人口比例（0.18）、
每千名老人床數（0.17）、高齡人口比例（0.17）、每千名老人機構數（0.12）、A 達成率（0.06）。
也就是「需求面」（老年與失能比例）比「供給面」（床位、機構數）更能決定一個縣市會不會供給不足。
結果存在 `ml_*.csv`，儀表板「供給不足預測(ML)」分頁也看得到。
